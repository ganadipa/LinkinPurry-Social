...
export class ConnectionController implements Controller {
    ...
    public registerRoutes(): void {
        ...
        this.hono.app.get("/api/connections", async (c) => {
            ...
            const targetUserId = userIdQuery ? BigInt(userIdQuery) : user?.id !== undefined ? BigInt(user.id) : undefined;
            
            if (!targetUserId) {
                throw new BadRequestException("User ID is undefined");
            }
            
            const connections = await this.connectionService.getConnections(targetUserId);
